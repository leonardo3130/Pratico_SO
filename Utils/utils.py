...
def get_files_and_directories(path, sottoalbero=True):
    ...
    if sottoalbero:
        ...
        process = subprocess.run(['find', path, '-maxdepth', '1'], capture_output=True, text=True)
...
def get_file_size(file_path):
    try:
        with open(file_path, 'rb') as file:
            ...
            size = file.tell()  # Ottieni la posizione corrente che rappresenta la dimensione del file
        return size
    except IOError as e:
        print(f"Error: {e}")
        return None
