/*
Rielaborare l'esercizio precedente per fare in modo che il vettore e le stringhe
dei nomi siano allocati con una unica operazione malloc (in modo da poter
liberare lo spazio usato da vreaddir con una unica operazione free.)
*/
...
char **vreaddir(const char *path) {
  ...
  while ((directory_entry = readdir(dir)) != NULL) {
    ...
    //(names + dir_len + 1) "salta" i puntatori alle stringhe
