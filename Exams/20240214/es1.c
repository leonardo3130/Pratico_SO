/*
Scrivere un programma search_name che deve cercare nel sottoalbero della
directory corrente tutti i file eseguibili con un nome file specifico passato
come primo e unico parametro indicando per ogni file il tipo di eseguibile
(script o eseguibile binario). Ad esempio il comando:
...
search_name deve stampare: . /testprog: script
...
int isELF(const char *filename) {
  ...
  if (memcmp(buf, "\x7f\x45\x4c\x46", 4) ==
      0)      // confronta i byte e ritorna 0 se coincidono
    return 1; // ELF
  return 0;
}
  ...
  while ((directory_entry = readdir(dir)) != NULL) {
    ...
    if (directory_entry->d_type == DT_DIR) {
      ...
      if (strcmp(file_name, directory_entry->d_name) == 0 &&
        ...
        } else { // file eseguibile o di script vuoto, ma con x on
          printf("%s: executable\n", new_path);
        }
      }
    }
